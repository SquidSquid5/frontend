...
+) sessionStorage는 탭을 닫으면 삭제, localStorage는 브라우저를 닫아도 유지이다.
+) 선택적으로 필요한 정보만 저장할 수 있다.

### ref.current.value = "~~" 로 값을 넣던 중 일어난 사건

- 할당 식의 왼쪽은 선택적 속성 액세스일 수 없습니다.
- 'nickNameRef.current'은(는) 'null'일 수 있습니다.
- Invalid assignment to `emailRef.current?.value`

해당 문제에 대해 다뤄보기 전 왜 이러한 문제가 일어났는가에 대해 먼저 설명해보겠다.

useRef를 사용할 때 초기값으로 null을 두곤 한다. 아래와 같은 이유 때문에...

1. DOM 노드가 아직 존재하지 않기 때문에
2. React가 마운트/언마운트 상태를 명확히 표현하기 위해
3. Typescript 타입 안정성
4. 임의 객체 생성보다 안전

컴포넌트가 처음 렌더링되는 시점에는 DOM 요소 생성 전이기 때문에 ref.current가 가리킬 대상이 없다. 따라서 마운트 후 DOM 엘리먼트가 할당되는 읽기 전용 렌더링을 보장하기 위함이다.

첫 번째 오류는 옵셔널 체이닝을 사용하여 값을 할당하려 할 때 발생한다.
current?.value 대신 if (current) current.value 하면 해결된다.

두 번째 오류는 null일 가능성이 있어서 useState를 업데이트할 수 없다는 것이다...

마지막은 부적절한 위치에서 할당 행위를 하려고 할 때 대표적인 발생하는 에러이다.

즉 3개의 오류를 동시에 해결하는 방법은 간단하다. current가 존재한다는 가정 하에 해당 코드들을 실행하면 되는 것이다...
...
지금은 한 파일 안에 fn과 컴포넌트, 변수들이 들어 있어 한 번 나눠야 한다는 생각에 인스턴스로의 분리를 진행하게 되었다.

문제는 response 에서 에러 메세지를 뱉고 있고 그걸 alert라는 컴포넌트에 전달을 해야 하는데 그게 쉽지않다는거임!!!!!!!!!!!!!!!!!!!!!!!!

instanse라는 객체? fn를 전역적으로 선언, service라는 폴더를 생성 후 안에 auth.ts를 만들어서 로그인 api를 만들어둠.... (오류 처리는 그 안에서 다 해서 필요 없을 줄 알았는데, alert 컴포넌트로 에러 메세지를 전달하려면 또 다시 호출 블록에서 try, catch를 해야 하고... 그 컴포넌트를 진짜로 호출한 곳에서 또 호출할 때 try catch를 해야 하는,,, 3단 try catch가 정말 의미가 있을까 싶은데)
